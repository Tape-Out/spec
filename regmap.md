...
| `reset` | 整数 | 复位值。**省略即无复位**——带复位触发器贵 52%，所以不默认给。负数按字段宽度取二补码，`-1` 即全一：宽度随参数变的字段（`spi` 的 `csdef` 宽 `csWidth`）写不出一个对每种宽度都成立的正数 |

**本版不实现**：`we`/`wel` · `hwclr` · `counter` · `singlepulse` · `intr`/`enable` 的标准中断聚合 · `external`。（别名寄存器一度也在这份名单里，后来实现了，见八之三。）
...
## 二之一、SystemRDL 没有、我们添的键

前一节那张表的名字与 SystemRDL 完全一致。下面这三个是**我们自己引申的**，
所以单列——写进同一张表会让人以为它们也是 SystemRDL 的词。

| 键 | 在哪一级 | 取值 | 为什么要它 |
|:--|:--:|:--|:--|
| `array` | 寄存器 | `{count, stride}`，`count` 可写旋钮名 | 一组同形寄存器只写一遍。`plic` 的每源优先级、每上下文使能都是它 |
| `width` | 寄存器 | 总线宽或它的两倍 | 比总线宽的寄存器要占两个地址 |
| `atomic` | 寄存器 | `latch-on-low` | **64 位寄存器挂 32 位总线**：读低半时把高半锁进影子，读高半返回影子。不这么做，两次读之间计数器会走，读出撕裂值。只对比总线宽的寄存器有意义，写在窄寄存器上报错 |
| `wrdata` | 字段 | `true` | 硬件要拿到**软件这一拍写进来的值**，而不是存下来的值。必须与 `swmod` 同用（总得先知道这一拍有没有写），且字段得软件可写 |
| `legal` | 字段 | 列表，元素是整数或 `{min, max}` 闭区间（区间可带 `feature`） | 写任意值、读回合法值（WARL）：非法写保持原值。见八之四 |

`array` 与 `width` 只是形状，`atomic`、`wrdata` 与 `legal` 是行为。**五个都会被校验**：
`array` 缺 `count` 或 `stride` 报错、多写一个键也报错；`width` 只收总线宽或两倍；
`atomic` 只收 `latch-on-low`；`wrdata` 缺 `swmod` 或字段软件写不进去都报错；
`legal` 的校验列在八之四。
...
## 八之三、`alias` —— 同一份存储的第二个地址

特权级会给同一份状态开两个地址：`sstatus` 是 `mstatus` 的**掩码视图**，只露 S 级看得到的那几位。没有这个词汇的话只能给 `sstatus` 单独开存储，而那样 M 侧与 S 侧各写各的，软件看到两份互不相干的状态——那不是别名，是错。

```yaml
  - name: sstatus
    offset: 0x100
    feature: smode
    alias: mstatus          # 共用 mstatus 的存储
    fields:                 # 只列 S 级看得到的那几位
      - { name: sie,  bits: "1:1", sw: rw, hw: r,  reset: 0 }
      - { name: spie, bits: "5:5", sw: rw, hw: rw, reset: 0 }
      - { name: spp,  bits: "8:8", sw: rw, hw: rw, reset: 0 }
```
...
| 约束 | 为什么 |
|:--|:--|
| 别名**不分配存储**，也不出接口方法 | 存储与方法在目标那边已经有了；再来一份就成了两份状态 |
| 字段必须与目标**同名同位** | 同一份存储只能有一个位置；名字对不上就说不清映到哪 |
| 目标不能是另一个别名 | 一层就够，两层只会让人猜 |
| 只列出来的字段可见 | 没列的读回零、写了不生效——「掩码视图」的含义就在这里 |
| 本版只支持标量寄存器 | 数组别名还没有用例，等真有再说 |

别名自己可以带 `feature`：`smode` 关掉时整个 `sstatus` 消失，而 `mstatus` 里那几位本来也被同一个 `feature` 门控着，两边一致。

### 怎么验它真的是同一份存储

**从一个地址写、从另一个地址读**。`hart` 的自检就是这么做的：
`csrrs zero, 0x100, t2` 写 `sstatus.sie`，`csrrs t2, 0x300, zero` 从 `mstatus` 读回来。读得到才算数；给别名单独开了存储的话这里就是零。

---

## 八之四、`legal` —— 写任意值，读回合法值（WARL）

有些字段只定义了一部分编码：`mtvec.MODE` 只有 0 与 1，`spi` 的帧长只到 8，交换机的老化时间是 10 到 1,000,000 秒。RISC-V 特权规范（20211203）第 2.3 节把这一类叫 **WARL**：*allow any value to be written while guaranteeing to return a legal value whenever read*，读回哪一个合法值 *should deterministically depend on the illegal written value and the architectural state*。没有这个词汇，寄存器照收非法值、读回的也是非法值，软件「写进去再读回来、看留没留住」的探测办法就失效了。

SystemRDL 2.0 没有对应的属性，`sw`、`hw`、`onwrite`、`precedence` 都不管取值范围。IP-XACT 有 `writeValueConstraint`，分列举值（`useEnumeratedValues`）与上下界（`minimum`/`maximum`）两种形态。我们的写法取这两种形态的并集。

```yaml
  - name: agetime
    offset: 0x10
    fields: [ { name: val, width: 20, sw: rw, hw: r, reset: 300,
                legal: [ { min: 10, max: 1000000 } ] } ]

  - name: mstatus
    offset: 0x300
    fields:
      - { name: mpp, bits: "12:11", sw: rw, hw: rw, reset: 3,
...
| 约束 | 为什么 |
|:--|:--|
| **非法写保持原值** | 802.1Q 对越界的老化时间是拒收；RISC-V 允许任何确定的合法值，保持原值满足它。另一种做法是退回列表里第一个值（pyrift 的 `warl`），那样写错一次就把正在用的配置换掉了 |
| 元素是整数或 `{min, max}` 闭区间；区间可带 `feature`，只在特性开着时算合法 | 列举与上下界是 IP-XACT 的两种形态；`mpp` 只收实现了的特权级，U 与 S 跟着 `smode` |
| 界写整数、参数名或「参数 ± 整数」，别的表达式报错 | `pinmux` 的 `sel` 上界是 `funcs - 1`，随参数变 |
| 必须给 `reset`，且复位值落在不依赖特性的合法值里 | 没有复位，上电那一刻读出来的就可能不合法 |
| 只用于软件能写的存储字段，不与 `volatile`、`onwrite`、`onread`、`hwset` 同用 | `legal` 管的是软件写进存储的值。硬件那一侧的写不受它约束，由 IP 自己保证 |
| 不写在别名上 | 别名共用目标的存储，也就共用目标的合法值 |
| 比总线宽的寄存器本版不支持 | 分两半写，半个值谈不上合法不合法 |
| 界放不进字段的位宽时报错 | 那样的界永远碰不到，多半是写错了 |

跨字段的约束不归这里管。`rcu` 的倍频与分频合不合法取决于输入时钟的频率（FE310-G002 手册表 12），不是哪一个字段自己的事。

### 怎么验

生成的一致性测试多一段：对每个区间的两端，**先写端点，再写紧挨着它的非法值，读回必须还是端点**。只看写全一与写全零，分不出「保持原值」与「退回复位值」，也分不出端点算没算进区间。

数组寄存器不进一致性测试，由各 IP 自己的测试台验：`pinmux` 给一针写一个不存在的功能号，读回必须还是原来的号，那一针照旧由原来那一路驱动。
...
`xirang gen --tb` 从 `regmap.yaml` 生成寄存器一致性测试：写全一读回可写位的掩码、写全零读回零、关掉的特性位读回零，带 `legal` 的字段写非法值不被收下（见八之四）。**类型检查看的是能不能建，调度门禁看的是会不会卡，这一道看的是读写对不对**——而生成的译码、掩码、字段落位正是最容易悄悄错的那部分。
